            ...
            <div class="grid xl:grid-cols-5 sm:grid-cols-3 grid-cols-2 gap-4 xl:ml-14 pt-6 pb-10 sm:mx-6 mx-4">
                @foreach ($tv as $tvItem)
                    @php
                        ...
                        $tvImg = "{$imageBaseURL}/w500{$tvItem->poster_path}";

                        // Ambil genre yang sesuai dengan id genre film
                        $genres = [];
                        foreach ($genre as $genreItem) {
                            if (in_array($genreItem->id, $tvItem->genre_ids)) {
                                $genres[] = $genreItem->name;
                            }
                        }
                    @endphp

                    <a href="{{ route('Tv-Show-Details', ['id' => $tvId]) }}" class="group">
                        <div
                            ...
                            <span
                                class="font-sans font-bold text-xl mt-4 line-clamp-1 md:line-clamp-none group-hover:line-clamp-none">{{ $tvTitle }}</span>
                            <div class="flex justify-between mt-2">
                                <div class="flex flex-row items-center">
                                    ...
                                    <span class="font-sans ml-2">{{ $tvRating }}</span>
                                </div>
                                <span class="font-sans">|</span>
                                <span class="font-sans">{{ $tvYear }}</span>
                            </div>
        ...
        <div class="mb-10">
            ...
            <div class="flex justify-center items-center gap-x-6">
                <button type="button"
                    ...
                    <a href="{{ $page > 1 ? route('Tv-Show', ['page' => $page - 1]) : '#' }}" aria-hidden="true"
                ...
                <button type="button"
                    class="min-h-[38px] min-w-[38px] py-2 px-2.5 inline-flex justify-center items-center gap-x-2 text-sm rounded-lg text-gray-800 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 disabled:opacity-50 disabled:pointer-events-none dark:text-black dark:hover:bg-white/10 dark:focus:bg-white/10">
                    <a href="{{ $page < $totalPages ? route('Tv-Show', ['page' => $page + 1]) : '#' }}"
