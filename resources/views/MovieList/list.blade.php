<div class="mt-12">
    ...
    <div class="grid xl:grid-cols-5 sm:grid-cols-3 grid-cols-2 gap-4 xl:ml-14 pt-6 pb-10 sm:mx-6 mx-4">
        @foreach ($popular as $popularItem)
            @php
                $original_date = $popularItem->release_date;
                $timestamp = strtotime($original_date);
                $movieYear = date('Y', $timestamp);

                $movieId = $popularItem->id;
                $movieTitle = $popularItem->title;
                $movieRating = $popularItem->vote_average;
                $movieImg = "{$imageBaseURL}/w500{$popularItem->poster_path}";

                // Ambil genre yang sesuai dengan id genre film
                $genres = [];
                foreach ($genre as $genreItem) {
                    if (in_array($genreItem->id, $popularItem->genre_ids)) {
